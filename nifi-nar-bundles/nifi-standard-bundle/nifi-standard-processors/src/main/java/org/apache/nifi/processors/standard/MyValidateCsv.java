...
public class MyValidateCsv extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final AtomicReference<Integer> recordID = new AtomicReference<Integer>(0);
                        ...
                        if (!isWholeFFValidation) {
                            invalidFF.set(session.append(invalidFF.get(), new OutputStreamCallback() {
                                ...
                                public void process(OutputStream out) throws IOException {
                                    out.write(print(headerList, csvPref, isFirstLineInvalid.get(), recordID.get()));
                                }
                            }));
                            validFF.set(session.append(validFF.get(), new OutputStreamCallback() {
                                @Override
                                public void process(OutputStream out) throws IOException {
                                    out.write(print(headerList, csvPref, isFirstLineValid.get(), recordID.get()));
                                }
                            }));
                        ...
                        try {
                            ...
                            if (!isWholeFFValidation && !stop) {
                                validFF.set(session.append(validFF.get(), new OutputStreamCallback() {
                                    @Override
                                    public void process(OutputStream out) throws IOException {
                                        out.write(print(list, csvPref, isFirstLineValid.get(), recordID.get()));
                                    }
                                }));
                            ...
                            if (isWholeFFValidation) {
                                ...
                                invalidFF.set(session.append(invalidFF.get(), new OutputStreamCallback() {
                                    @Override
                                    public void process(OutputStream out) throws IOException {
                                        out.write(print(e.getCsvContext().getRowSource(), csvPref, isFirstLineInvalid.get(), recordID.get()));
                                    }
                                }));
                                logValidationErrors(flowFile, recordID.get(), e.getLocalizedMessage());
     ...
     */
    private byte[] print(List<?> list, CsvPreference csvPref, boolean isFirstLine, int record_id) {
        StringBuffer buffer = new StringBuffer();
    ...
    private void logValidationErrors(final FlowFile flowFile, final int recordCount, String errorMsg) {
